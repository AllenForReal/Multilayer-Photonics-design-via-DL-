...
if __name__ == '__main__':
    ...
    wlmin = 300
    wlmax = 2500
    # tmin = torch.tensor([50, 50, 50, 15, 50, 50, 50, 50]).to(device)
    # tmax = torch.tensor([200, 220, 200, 30, 200, 200, 200, 200]).to(device)
    # tmin = torch.tensor([10, 10, 10, 10, 10, 10, 10, 10]).to(device)
    # tmax = torch.tensor([300, 300, 300, 50, 300, 300, 300, 300]).to(device)
    x0 = torch.tensor([89, 59, 116, 140, 17, 180, 121, 162])
    d = 30
    tmin = torch.maximum(x0 - d, torch.zeros(layer_dim)).to(device)
    tmax = (x0 + d).to(device)
    tm = tmm(mat_lst=['air', 'TiO2', 'SiO2', 'SiC', 'SiO2', 'Ag', 'TiO2', 'MgF2', 'TiO2', 'SiO2'],
