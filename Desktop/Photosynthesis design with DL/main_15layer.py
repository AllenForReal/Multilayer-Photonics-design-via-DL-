...
def train(net, optimizer, batch=10):
    ...
    for idx in range(batch):
        ...
        # bloss = torch.sum(relu(recons - tmax) + relu(tmin - recons))
        preds = tm(recons)
...
if __name__ == '__main__':
    ...
    wlmin = 300
    wlmax = 2500
    x0 = torch.tensor([196, 147, 89, 205, 60, 243, 187, 18, 119, 0, 108, 12, 105, 95, 80])
    d = 50
    tmin = torch.maximum(x0 - d, torch.zeros(layer_dim)).to(device)
    tmax = (x0 + d).to(device)
    # x0 = torch.tensor([91.1, 51.6, 190.3, 19.8, 154.1, 109.3])
    tm = tmm(mat_lst=['air', 'Al2O3', 'TiO2', 'MgF2', 'SiO2', 'SiC', 'MgF2', 'SiO2', 'Ag',
                      'Al2O3', 'TiO2', 'SiC', 'Al2O3', 'MgF2', 'SiC', 'HfO2', 'SiO2'],
